...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.heads = nn.ModuleList([
                AttentionHead(
                    d_model,
                    d_model // num_attention_heads,  # d_k
                    d_model // num_attention_heads,  # d_q
                    d_model // num_attention_heads   # d_v
                )
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        z1 = self.layer_norm_1(x + self.self_attention(x, mask))
        z2 = self.layer_norm_2(z1 + self.feed_forward(z1))
        return z2
